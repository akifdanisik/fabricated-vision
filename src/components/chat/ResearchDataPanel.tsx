...
export default function ResearchDataPanel({ 
  ...
  return (
    <div className="border-t border-gray-200 bg-white transition-all duration-300 ease-in-out rounded-b-xl overflow-hidden">
      <div className="px-4 py-2 flex items-center justify-between border-b border-gray-200 bg-gray-50">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-2">
            <Badge variant="outline" className="text-xs bg-gray-100 rounded-full">
        ...
        <div className="p-4">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-2 mb-4 rounded-lg">
              <TabsTrigger value="documents" className="flex items-center gap-1 rounded-l-lg">
                <FileText className="h-4 w-4" />
                <span>Documents</span>
                <Badge variant="outline" className="ml-1 text-xs bg-gray-100 rounded-full">
                  {documents.length}
                </Badge>
              </TabsTrigger>
              <TabsTrigger value="suppliers" className="flex items-center gap-1 rounded-r-lg">
            ...
            <TabsContent value="documents" className="mt-0">
              <div className="overflow-auto max-h-[300px]">
                {isLoading ? (
                  ...
                  <div className="rounded-xl overflow-hidden border border-accent-pale/50">
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-accent-pale/10">
                          <TableHead className="w-[250px] rounded-tl-lg">Document</TableHead>
                          <TableHead className="w-[120px]">Date</TableHead>
                          <TableHead className="w-[150px]">Document Type</TableHead>
                          <TableHead>Investment Risks</TableHead>
                          <TableHead className="rounded-tr-lg">Market Considerations</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {documents.map((doc) => (
                          <TableRow key={doc.id}>
                            ...
                            <TableCell>
                              <Badge className={cn("font-normal rounded-full", documentTypeColors[doc.documentType] || "bg-gray-100")}>
                                {doc.documentType}
                              </Badge>
                            </TableCell>
                  ...
                  <div className="text-center py-8 text-gray-500 rounded-xl border border-accent-pale/30 bg-accent-pale/5">
